...
GROUP <- GRIN_23 %>% 
  filter(Group != "Bare") %>%
  select(ID, Fire, Group, Species, Coverage) %>% 
  group_by(ID, Fire, Group)  %>% 
  summarize(TotalCV = sum(Coverage))

Woody = filter(GROUP, Group == "Woody")

# Test for Significance #
Fun_anova = aov(TotalCV ~ Fire, data = GROUP)
summary(Fun_anova)
...
Group_Box = 
  ggplot(GROUP, aes(x = Fire, y = TotalCV, fill = Group)) +
