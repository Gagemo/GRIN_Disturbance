...
data = filter(GRIN, Species == "Aristida stricta" | 
...
Grass_22 = select(GRIN_22M, Grass) %>%
  cbind(Treat_22)

# Check Assumptions #
model  <- lm(Grass ~ Fire, data = Grass_22)
...
Grass_22$Fire = as.factor(Grass_22$Fire)
Grass_22 %>% levene_test(Grass ~ Fire)

# Test for Significance #
anova_22 = Grass_22 %>% kruskal_test(Grass ~ Fire) %>% 
  add_significance()
summary(anova_22)

tukey_22 <- Grass_22 %>% 
  dunn_test(Grass ~ Fire) %>% 
  add_significance() %>% 
  add_xy_position()
tukey_22
...
Grass_23 = select(GRIN_23M, Grass) %>%
  cbind(Treat_23)
Grass_23 <- ungroup(Grass_23)

# Check Assumptions #
model  <- lm(Grass ~ Fire, data = Grass_23)
# Create a QQ plot of residuals
ggqqplot(residuals(model))
# Compute Shapiro-Wilk test of normality
shapiro_test(residuals(model))
plot(model, 1)
# Compute Levene's Test
Grass_23$Fire = as.factor(Grass_23$Fire)
Grass_23 %>% levene_test(Grass ~ Fire)

# Test for Significance #
anova_23 = Grass_23 %>% kruskal_test(Grass ~ Fire) %>% 
  add_significance()
summary(anova_23)
...
################################################################################
################ Create Box Plot for Seeded Grasss Across Years #################
################################################################################

## Lovegrass Coverage 2022 Box plot ##
Box22 = 
  ggplot(Grass_22, aes(x = Fire, y = Grass), colour = Fire) +
  ...
  labs(x = "", y = "Seeded Grass % Coverage", title = "2022")
Box22

ggsave("Figures/Chapter 2 - Fire/seedgrassbox22.png", 
       width = 12, height = 8)

## Lovegrass Coverage 2023 Boxplot ##
Box23 = 
  ggplot(Grass_23, aes(x = Fire, y = Grass), colour = Fire) +
  geom_boxplot(aes(fill=Fire), alpha = 0.5, outlier.shape = NA) +
  geom_point(aes(fill=Fire), 
             position = position_jitterdodge(), size = 2, alpha = 0.5) +
...
ggsave("Figures/Chapter 2 - Fire/seedgrassbox23.png", 
       width = 12, height = 8)

################## Save Figures Above using ggarrange ##########################
ggarrange(Box22, Box23, ncol = 2, nrow = 1)
ggsave("Figures/Chapter 2 - Fire/22-23_grassseedBox.png", 
       width = 12, height = 10)
